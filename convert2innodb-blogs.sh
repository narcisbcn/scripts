...
mysql -N -B -e "SELECT table_schema, table_name FROM INFORMATION_SCHEMA.TABLES   WHERE engine = 'MyISAM' AND table_schema  in ('wordpress', 'wordpressBU', 'wordpress_predevmove');" |awk '{print $1"."$2}' > ${TMP_FILE}
