...
def fetch_all(host, user, password='', port='' ):
  ...
  try:
    ...
    cursor = conn.cursor()
    cursor.execute("select table_schema, table_name, column_name, column_type, column_key from columns where table_schema not in ('information_schema', 'mysql', 'maatkit', 'percona') and column_type like '%int%';")
